 ...
 * @param[in/out] bufferSize 日志路径大小
 * @return
 * - ITKSTATUS_OK
 * - ITKSTATUS_INSUFFICIENT_RESOURCES
*/
 ...
 * @param[in/out] bufferSize Log path size
 * @return
 * - ITKSTATUS_OK
 * - ITKSTATUS_INSUFFICIENT_RESOURCES
 */
...
\~chinese 
 * @brief 枚举设备
 * @param[in] nDevType 设备类型, 参考 MANAGER_TYPE_LIST 。当nDevType为-1，该接口将枚举所有类型设备。
 * @param[out] pDevList 设备信息列表,包含枚举的设备数量与设备信息。
 * @return
 * - ITKSTATUS_OK
 * @see MANAGER_TYPE_LIST
*/
 /**
\~english 
 ...
 * @note 在调用本函数前必须先调用 ItkManGetDeviceCount 或 ItkManGetDeviceCountEx 。
 * @see ItkManGetDeviceCount ItkManGetDeviceCountEx
*/
 /**
\~english 
 ...
 * - ITKSTATUS_INVALID_ARG
 * - ITKSTATUS_DEVICE_ID_OUTOF_RANGE
 * @note Before calling this function, ItkManGetDeviceCount or ItkManGetDeviceCountEx must be called first.
 * @see ItkManGetDeviceCount ItkManGetDeviceCountEx
 */
 ...
 * - ITKSTATUS_INVALID_ARG
 * - ITKSTATUS_ARG_OUT_OF_RANGE
 * @note Readable global parameters prm are shown as follows:
 ...
 * - ITKSTATUS_INVALID_ARG
 * - ITKSTATUS_INVALID_HANDLE
 * @note Writable global parameters prm are shown as follows:
 ...
 * - ITKSTATUS_OK
 * - ITKSTATUS_INVALID_HANDLE
 * @note This function must indicate the full path of the file. Applicable to  postfix.
 * @see ItkDevGetUpgradeProcess
 */
 ...
 * @brief 获取相机升级进度
 * @param[in] hDev 相机设备句柄
 * @param[out] pnProcess 相机升级进度：0-300。0-100：擦除进度，101-200：写入进度，201-300：升级进度
 * @return
 * - ITKSTATUS_OK
 * - ITKSTATUS_INVALID_HANDLE
 * @see ItkDevUpgrade
 ...
 * @param[in] hDev Camera device handle
 * @param[out] pnProcess the process of device upgrade: 0-300. 0-100: Erase progress, 101-200: Write progress，201-300: Upgrade progress
 * @return
 * - ITKSTATUS_OK
 * - ITKSTATUS_INVALID_HANDLE
 * @see ItkDevUpgrade
 ...
 * - ITKSTATUS_INVALID_ARG
 * - ITKSTATUS_INVALID_HANDLE
 * @note Camera Link camera has no data stream channel.
 * @see ItkDevAllocStreamEx
 */
 ...
 * - ITKSTATUS_INVALID_ARG
 * - ITKSTATUS_INVALID_HANDLE
 * @note The input items of pEventName are as follows:
 * | pEventName           |
 * |----------------------|
 * | DeviceRemove         |
 * | FeatureValueChanged  |
 ...
 * - ITKSTATUS_INVALID_ARG
 * - ITKSTATUS_INVALID_HANDLE
 * @note Readable device parameters prm are shown as follows:
 ...
 * - ITKSTATUS_INVALID_ARG
 * - ITKSTATUS_INVALID_HANDLE
 * @note Writable device parameters prm are shown as follows:
 ...
 * 1. 调用此API之前，必须先枚举当前电脑上可用相机的数量。
 * 2. pActionCmdResults仅对当次调用生效。若多次调用该函数，则之前返回的pActionCmdResults失效。
*/
 /**
\~english 
 * @brief Set force IP address for gigabit network camera
 ...
 * 1. Before calling this API, you must enumerate the number of cameras available on the current computer.
 * 2. pActionCmdResults is only valid for current call. When this api is called multiple times, pActionCmdResults returned before become invalid.
 */
IKAPC_API ITKSTATUS IKAPC_CC
 ...
 * - ITKSTATUS_NOT_IMPLEMENT
 * - ITKSTATUS_INCOMPATIBLE_FEATURE_TYPE
 * @note 设定值的取值范围一定是[ Min, Max ]，取值同时也应满足value = Min + N*Inc ，N=0,1,2,…
 ...
 * - ITKSTATUS_NOT_IMPLEMENT
 * - ITKSTATUS_INCOMPATIBLE_FEATURE_TYPE
 * @note The range of values for the set value must be [ Min ,Max ], 
 ...
 * @brief 获取U3V的传输包大小
 * @param[in] hDev 相机设备句柄
 * @param[in] nTransferSize 传输的包大小指针，单位Byte
 * @return
 * - ITKSTATUS_OK
 * - ITKSTATUS_INVALID_HANDLE
 * - ITKSTATUS_NOT_IMPLEMENT
 ...
 * @param[in] hDev Camera device handle
 * @param[in] nTransferSize Transfer size, unit is Byte
 * @return
 * - ITKSTATUS_OK
 * - ITKSTATUS_INVALID_HANDLE
 * - ITKSTATUS_NOT_IMPLEMENT
 ...
 * @brief 设置U3V的传输包大小
 * @param[in] hDev 相机设备句柄
 * @param[in] nTransferSize 传输的包大小，单位Byte，默认为0x400000，范围[ 0x400, 0x400000]
 * @return
 * - ITKSTATUS_OK
 * - ITKSTATUS_INVALID_ARG
 * - ITKSTATUS_INVALID_HANDLE
 * - ITKSTATUS_NOT_IMPLEMENT
 * @note 接口仅U3V协议的USB相机支持。增加传输包大小可以适当降低取流时的CPU占用率。
 ...
 * @param[in] hDev Camera device handle
 * @param[in] nTransferSize Transfer size, unit is Byte，default 0x400000，rang in [ 0x400, 0x400000]
 * @return
 * - ITKSTATUS_OK
 * - ITKSTATUS_INVALID_ARG
 * - ITKSTATUS_INVALID_HANDLE
 ...
 * @param[in] hStream 数据流句柄
 * @param[out] pnBufferNum 当前缓冲区列表buffer个数
 * @return
 * - ITKSTATUS_OK
 * - ITKSTATUS_INVALID_ARG
 * - ITKSTATUS_INVALID_HANDLE
 ...
 * @param[in] hStream Data stream handle
 * @param[out] pnBufferNum Buffer number of the current bufferList
 * @return
 * - ITKSTATUS_OK
 * - ITKSTATUS_INVALID_ARG
 * - ITKSTATUS_INVALID_HANDLE
 ...
 * @param[in] hStream 数据流句柄
 * @param[out] pnValidBufferNum 当前缓冲区有效buffer个数
 * @return
 * - ITKSTATUS_OK
 * - ITKSTATUS_INVALID_ARG
 * - ITKSTATUS_INVALID_HANDLE
 ...
 * @param[in] hStream Data stream handle
 * @param[out] pnValidBufferNum Valid buffer number of the current bufferList
 * @return
 * - ITKSTATUS_OK
 * - ITKSTATUS_INVALID_ARG
 * - ITKSTATUS_INVALID_HANDLE
 ...
 * - ITKSTATUS_INVALID_ARG
 * - ITKSTATUS_INVALID_HANDLE
 * @note Readable data stream parameters prm are shown as follows:
 ...
 * - ITKSTATUS_INVALID_ARG
 * - ITKSTATUS_INVALID_HANDLE
 * @note Writable data stream parameters prm are shown as follows:
 ...
 * @note  Get buffer that indacate current frame in end-of-frame callback. Use this function to get correct value only in end-of-frame callback.
 * @return
 * - ITKSTATUS_OK
 * - ITKSTATUS_INVALID_ARG
 * - ITKSTATUS_INVALID_HANDLE
 ...
 * @brief 清空数据流回调函数
 * @param[in] hStream 数据流句柄
 * @param[in] eventType 回调事件类型，具体参见 ITKSTREAM_PRM_SUPPORT_EVENT_TYPE 
 * @return
 * - ITKSTATUS_OK
 * - ITKSTATUS_INVALID_ARG
 * - ITKSTATUS_INVALID_HANDLE
 * @note 回调事件类型eventType可选项如下所示。不同型号的相机支持的回调事件类型不同，具体请以实际情况为准：
 * | 回调事件类型eventType                          |
 * |------------------------------------------------|
 * | ITKSTREAM_VAL_EVENT_TYPE_START_OF_STREAM       |
 * | ITKSTREAM_VAL_EVENT_TYPE_END_OF_STREAM         |
 ...
 * @param[in] hStream Data stream handle
 * @param[in] eventType Callback event type, see ITKSTREAM_PRM_SUPPORT_EVENT_TYPE
 * @return
 * - ITKSTATUS_OK
 * - ITKSTATUS_INVALID_ARG
 * - ITKSTATUS_INVALID_HANDLE
 * @note The callback event types eventType are as follows. Different camera models support different callback event types. Please refer to the actual situation for details:
 * | Callback event types eventType                          |
 * |------------------------------------------------|
 * | ITKSTREAM_VAL_EVENT_TYPE_START_OF_STREAM       |
 * | ITKSTREAM_VAL_EVENT_TYPE_END_OF_STREAM         |
 ...
 * @param[in] format 缓冲区像素格式，参见 ITKBUFFER_FORMAT
 ...
 * @param[in] format Buffer pixel format, see ITKBUFFER_FORMAT
 ...
 * @param[in] height 缓冲区高度，单位像素
 * @param[in] format 缓冲区像素格式，参见 ITKBUFFER_FORMAT
 * @param[in] payloadType Chunk数据类型，参见 PAYLOAD_TYPE
 * @param[out] phBuffer 缓冲区句柄
 * @return
 * - ITKSTATUS_OK
 * - ITKSTATUS_INVALID_ARG
 ...
 * @param[in] height Buffer height, unit pixel
 * @param[in] format Buffer pixel format, see ITKBUFFER_FORMAT
 * @param[in] payloadType Chunk data type, see PAYLOAD_TYPE
 * @param[out] phBuffer Buffer handle
 * @return
 * - ITKSTATUS_OK
 * - ITKSTATUS_INVALID_ARG
...
/**
\~chinese 
 * @brief 获取缓冲区PART信息, 仅在缓冲区负载为 MULTI_PART_PAYLOAD_TYPE 时使用
 * @param[in] hBuffer 缓冲区句柄
 ...
 * - ITKSTATUS_INVALID_ARG
 * - ITKSTATUS_INVALID_HANDLE
 * @note 缓冲区偏移量和用户缓冲区大小之和不能超过数据缓冲区的大小。
 * @see ItkBufferWrite
*/
 /**
\~english 
 ...
 * - ITKSTATUS_INVALID_ARG
 * - ITKSTATUS_INVALID_HANDLE
 * @note The sum of buffer offset and user buffer size cannot exceed the size of data buffer.
 * @see ItkBufferWrite
 */
 ...
 * @see ITKBUFFER_VAL_BAYER_PLANNAR
*/
 /**
\~english 
 ...
 * - ITKSTATUS_INVALID_HANDLE
 * - ITKSTATUS_INVALID_ARG
 * @note The image formats available for saving are optional as follows:
 ...
 * - ITKSTATUS_INVALID_HANDLE
 * - ITKSTATUS_INVALID_ARG
 * @note 如果buffer的格式是可以通过 ITKBUFFER_VAL_CONVERT_OPTION_AUTO_FORMAT 自动转换的, 则会返回true。
*/
 /**
\~english 
 ...
 * - ITKSTATUS_INVALID_HANDLE
 * - ITKSTATUS_INVALID_ARG
 * @note 缓冲区数据格式仅支持Mono8、Mono10、Mono12和Bayer8、Bayer10、Bayer12。
*/
/**
\~english
 ...
 * - ITKSTATUS_INVALID_HANDLE
 * - ITKSTATUS_INVALD_ARG
 * @note Writable view window parameters prm are as follows:
 ...
 * - ITKSTATUS_INVALID_HANDLE
 * - ITKSTATUS_INVALD_ARG
 * @note Readable view window parameters prm are as follows:
